...
func MockData() {
	...
	*/
	...
	if _, err := os.Stat(GConfig.Processor.LogPath); err != nil && os.IsNotExist(err) {
		_, _ = os.Create(GConfig.Processor.LogPath)
		fmt.Println("初始化日志日志文件成功")
	}

	file, err := os.OpenFile(GConfig.Processor.LogPath, os.O_WRONLY|os.O_APPEND, os.ModePerm)
	if err != nil {
